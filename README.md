...
- 任務：討伐任務進度與獎勵。
- 世界：國家勢力、城鎮、排行榜展示。
- 本機帳號：帳密只在此瀏覽器驗證，密碼保存為加鹽 SHA-256 摘要；不同帳號使用獨立角色存檔。舊版未驗證 session 會失效，必須重新註冊或登入後才會移轉舊存檔。
- 存檔：localStorage 自動保存、上一份有效備份、嚴格 JSON 匯入驗證與舊版存檔移轉；損壞主檔會顯示原始內容，必須修復匯入或明確捨棄，不會被新角色自動覆寫。舊檔若缺少一次性永久道具紀錄，遷移時會保守標記為已使用，避免升級後重複加成。
- RWD：桌機與手機都可操作。
